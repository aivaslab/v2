...
class ToMesh:
    def __init__(self, rot, tr):
        """ Load mesh and apply rotation and translation to the mesh

        Args:
            rot: List[z+, y+, x+]
                The rotation degree around the z+, y+, x+ axis
            tr: List[x+, y+, z+]
                The translation distance along the x+, y+, z+ axis
        """
        self.rot = rot
        self.tr = tr
    ...
    def __call__(self, path):
        ...
        mesh.fix_normals()
        mesh.fill_holes()
        # mesh.remove_duplicate_faces()  # Sometimes will remove faces that are actually not duplicated
    ...
    def apply_tr(self, mesh):
        mesh.apply_translation(self.tr)
        return mesh
